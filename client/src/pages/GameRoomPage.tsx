...
function DroppableDiscard({ id, children, canDrop }: { id: string; children: React.ReactNode; canDrop: boolean }) {
	const { setNodeRef, isOver } = useDroppable({ id, disabled: !canDrop });
	return (
		<div ref={setNodeRef} className={`text-center rounded-lg transition-all ${canDrop ? "border-2 border-dashed" : ""} ${isOver ? "border-blue-400 bg-blue-50 dark:bg-blue-950" : "border-gray-400 dark:border-gray-500"}`}>
			{children}
		</div>
	);
}
...
export default function GameRoomPage() {
	...
	const playerRank = new Map(sortedByScore.map((p, i) => [p.sessionId, i]));
			...
			{status === "playing" && phase !== "waiting" && (
				<DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
					<div className="flex flex-col" style={{ minHeight: "calc(100vh - 180px)" }}>
						{/* Game table — fills remaining height */}
						<div className="flex-1 relative">
						{(() => {
							const opps = players.filter((p) => p.sessionId !== mySessionId);
							const oppRow = (o: PlayerState, isLeft: boolean) => {
								const isActive = o.sessionId === currentPlayer?.sessionId;
								const r = playerRank.get(o.sessionId)! + 1; const rl = rankLabels[r - 1] || `${r}th`;
								const mg = new Map<string, CardData[]>();
								for (const c of o.board) { if (!c.meldGroupId) continue; const g = mg.get(c.meldGroupId); if (g) g.push(c); else mg.set(c.meldGroupId, [c]); }
								...
								return (
									<div className={`flex items-center ${isLeft ? "flex-row" : "flex-row-reverse"}`}>
										{/* Profile — always horizontal */}
										<div className={`flex flex-col gap-1.5 p-2 rounded-lg border shrink-0 ${isActive ? "bg-blue-50 dark:bg-blue-950 ring-1 ring-blue-300 border-blue-300 dark:border-blue-600" : "border-gray-200 dark:border-gray-700"}`}>
											<div className="flex items-center gap-2">
												<div className={`w-7 h-7 rounded-full ${isLeft ? "bg-blue-500" : "bg-purple-500"} flex items-center justify-center text-white text-xs font-bold shrink-0`}>{o.name.charAt(0).toUpperCase()}</div>
												<div className="text-center"><p className="text-xs font-semibold leading-tight">{o.name}</p><p className="text-[10px] text-gray-500 dark:text-gray-400">{rl} — {o.score}</p></div>
												<div className={`w-2 h-2 rounded-full shrink-0 ${o.disconnected ? "bg-red-500" : "bg-green-500 animate-pulse"}`} />
											</div>
										...
										{/* Fixed-size wrapper for rotated cards — prevents layout blowout */}
										<div className="relative overflow-visible shrink-0" style={{ width: 80, height: 300 }}>
											<div className="absolute flex flex-row items-center gap-2 origin-center" style={{ left: "50%", top: "50%", transform: `translate(-50%, -50%) ${isLeft ? "rotate(90deg)" : "rotate(-90deg)"}` }}>
												<div className="flex gap-0.5">{o.hand.map((_, i) => <Card key={i} faceDown small />)}</div>
								...
								<>
									...
									<div className="flex items-center justify-center h-full">
										<div className="flex flex-col items-center gap-6">
											...
											{/* Central meld zone — large */}
											<DroppableBoard id="board" canDrop={canMeld}>
												{myBoard.length > 0 ? (
													<div className="flex flex-wrap gap-2 justify-center min-h-[150px] items-center">
														{(() => {
															const mg = new Map<string, CardData[]>();
													...
													<div className="min-h-[150px] flex items-center justify-center">
														<p className="text-xs text-gray-400 dark:text-gray-500">{canMeld ? "Drag cards here to meld" : "No melds yet"}</p>
													</div>
												)}
											</DroppableBoard>
										</div>
					...
					<div className="flex flex-col items-center gap-3 pt-4 pb-2">
						...
						{(() => {
							...
							if (!me) return null;
							const r = playerRank.get(me.sessionId)! + 1; const rl = rankLabels[r - 1] || `${r}th`;
							...
							return (
								<div className={`flex flex-col gap-1.5 p-2 rounded-lg border ${myActive ? "bg-blue-50 dark:bg-blue-950 ring-1 ring-blue-300 border-blue-300 dark:border-blue-600" : "border-gray-200 dark:border-gray-700"}`}>
									<div className="flex items-center gap-2">
										<div className="w-7 h-7 rounded-full bg-green-600 flex items-center justify-center text-white text-xs font-bold shrink-0">{me.name.charAt(0).toUpperCase()}</div>
										<div className="text-center"><p className="text-xs font-semibold leading-tight">{me.name}</p><p className="text-[10px] text-gray-500 dark:text-gray-400">{rl} — {me.score}</p></div>
